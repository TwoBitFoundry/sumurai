...
Production deployments must provision a publicly trusted nginx server certificate before Sumurai is exposed to users. Local development may use the generated self-signed certificate, but production startup now fails when nginx would serve missing, self-signed, or soon-expiring certificate material.
...
- `ENVIRONMENT=production`: explicit production runtime signal.
...
- `HTTP_PORT=80`, `HTTPS_PORT=443`, and `SSL_PORT=443` for public production deployments.
...
```bash
DOMAIN=app.example.com ENVIRONMENT=production HTTP_PORT=80 HTTPS_PORT=443 SSL_PORT=443 docker compose up -d nginx
```
...
DOMAIN=app.example.com ENVIRONMENT=production HTTP_PORT=80 HTTPS_PORT=443 SSL_PORT=443 docker compose restart nginx
```
...
```bash
DOMAIN=app.example.com ENVIRONMENT=production HTTP_PORT=80 HTTPS_PORT=443 SSL_PORT=443 docker compose ps nginx
```

Verify compose renders the production port mapping:

```bash
DOMAIN=app.example.com ENVIRONMENT=production HTTP_PORT=80 HTTPS_PORT=443 SSL_PORT=443 docker compose config
```
...
Development deployments using `DOMAIN=localhost` may generate a 1-day self-signed certificate for local bootstrapping. That path is not acceptable for production traffic.
