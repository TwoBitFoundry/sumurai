...
- **Backend layering.** `handlers` → `services` → `repository_service` (SQLx) / `cache_service` (Redis) / `providers/*`. Domain types in `models/`. Tests in `backend/src/tests/`. Cross-cutting middleware in `middleware/` + `auth_middleware.rs`.
