  ...
  it('Given existing budget; When edit amount; Then updates UI immediately and reconciles with server', async () => {
    fetchMock = installFetchRoutes({
      ...
      'PUT /api/budgets*': (req: Request) => {
        ...
        return new Response(JSON.stringify({ id: req.url.split('/').pop(), category: 'Food', amount: '275' }), { status: 200, headers: { 'Content-Type': 'application/json' } })
      },
      'GET /api/transactions': [],
      'GET /api/analytics/spending*': 0,
      'GET /api/analytics/categories*': [],
    ...
    await waitFor(() => {
      const putCall = fetchMock.mock.calls.find(c => String(c[0]).startsWith('/api/budgets/') && (c[1]?.method || 'GET').toUpperCase() === 'PUT')
