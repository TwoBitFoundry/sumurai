...
jest.mock('@/features/plaid/components/ProviderSelectionPanel', () => ({
  ProviderSelectionPanel: ({
    ...
    const providers = visibleProviders ?? (['diy', 'simplefin', 'teller', 'plaid'] as const);
    return (
...
describe('AccountsPage', () => {
  ...
  it('updates the bank status to re-auth needed when sync fails with auth error', async () => {
    const user = userEvent.setup();
    ...
    jest
      ...
      .mockRejectedValueOnce(new ApiError(403, 'Auth required'));
