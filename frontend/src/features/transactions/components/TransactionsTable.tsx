        ...
        <>
          <div className="overflow-x-auto">
            <table className={cn('min-w-full', 'text-sm', 'table-fixed')}>
              <thead className={cn('bg-slate-200', 'text-slate-700', 'transition-colors', 'duration-500', 'dark:bg-slate-700', 'dark:text-slate-300')}>
                <tr className={cn('border-b', 'border-slate-300', 'dark:border-slate-600')}>
                  <th className={cn('w-[15%]', 'whitespace-nowrap', 'px-4', 'py-3', 'text-left', 'text-xs', 'font-semibold', 'uppercase', 'tracking-[0.18em]')}>Date</th>
                  <th className={cn('w-[30%]', 'px-4', 'py-3', 'text-left', 'text-xs', 'font-semibold', 'uppercase', 'tracking-[0.18em]')}>Merchant</th>
                  <th className={cn('w-[15%]', 'whitespace-nowrap', 'px-4', 'py-3', 'text-right', 'text-xs', 'font-semibold', 'uppercase', 'tracking-[0.18em]')}>Amount</th>
                  <th className={cn('w-[20%]', 'whitespace-nowrap', 'px-4', 'py-3', 'text-left', 'text-xs', 'font-semibold', 'uppercase', 'tracking-[0.18em]')}>Account</th>
                  <th className={cn('w-[20%]', 'whitespace-nowrap', 'px-4', 'py-3', 'text-left', 'text-xs', 'font-semibold', 'uppercase', 'tracking-[0.18em]')}>Category</th>
                </tr>
              </thead>
              <AnimatePresence mode="wait" initial={false}>
                ...
                >
                  ...
                  return (
                    ...
                    >
                      <td className={cn('relative', 'whitespace-nowrap', 'px-4', 'py-3', 'align-middle', 'text-slate-900', 'transition-colors', 'duration-500', 'dark:text-white')}>
                        {new Date(r.date).toLocaleDateString()}
                      </td>
                      <td className={cn('truncate', 'px-4', 'py-3', 'align-middle')} title={r.name || r.merchant || '-'}>
                        <span className={cn('block', 'truncate', 'font-medium', 'text-slate-900', 'transition-colors', 'duration-500', 'dark:text-white')}>
                          {r.name || r.merchant || '-'}
                        </span>
                      </td>
                      ...
                      <td className={cn('whitespace-nowrap', 'px-4', 'py-3', 'align-middle')}>
                        <span className={cn('text-xs', 'text-slate-600', 'transition-colors', 'duration-500', 'dark:text-slate-400')}>
                          {r.account_name}
                          {r.account_mask && (
                            <span className={cn('ml-1', 'text-slate-400', 'transition-colors', 'duration-500', 'dark:text-slate-500')}>
                              ••••{r.account_mask}
                            </span>
                          )}
                        </span>
          ...
          <div className={cn('flex', 'items-center', 'justify-between', 'border-t', 'border-slate-200/70', 'bg-slate-50/50', 'px-4', 'py-4', 'transition-colors', 'duration-500', 'dark:border-slate-700/50', 'dark:bg-slate-800/30')}>
            <div className={cn('text-xs', 'text-slate-600', 'transition-colors', 'duration-500', 'dark:text-slate-400')}>
