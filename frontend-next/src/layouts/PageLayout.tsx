...
export function PageLayout({
  ...
  return (
    <div className={cn('space-y-8', className)}>
      <section className={cn('relative', 'overflow-hidden', 'rounded-[2.25rem]', 'border', 'border-white/35', 'bg-white/24', 'p-8', 'shadow-[0_32px_110px_-60px_rgba(15,23,42,0.75)]', 'backdrop-blur-[28px]', 'backdrop-saturate-[150%]', 'transition-colors', 'duration-500', 'ease-out', 'dark:border-white/12', 'dark:bg-[#0f172a]/55', 'dark:shadow-[0_36px_120px_-62px_rgba(2,6,23,0.85)]', 'sm:p-12')}>
        <div className={cn('pointer-events-none', 'absolute', 'inset-0')}>
          <div className={cn('absolute', 'inset-[1px]', 'rounded-[2.2rem]', 'ring-1', 'ring-white/45', 'shadow-[inset_0_1px_0_rgba(255,255,255,0.45),inset_0_-1px_0_rgba(15,23,42,0.18)]', 'dark:ring-white/12', 'dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.12),inset_0_-1px_0_rgba(2,6,23,0.48)]')} />
        ...
        <div className={cn('relative', 'z-10', 'flex', 'flex-col', 'gap-5')}>
          <div className={cn('flex', 'flex-col', 'gap-5', 'lg:flex-row', 'lg:items-start', 'lg:justify-between')}>
            <div className={cn('max-w-2xl', 'space-y-3')}>
              ...
              <div className="space-y-2">
                <h1 className={cn('text-3xl', 'font-bold', 'text-slate-900', 'transition-colors', 'duration-300', 'ease-out', 'dark:text-white', 'sm:text-4xl')}>
                  {title}
                </h1>
                {subtitle && (
          ...
          {error && (
            <div className={cn('rounded-2xl', 'border', 'border-red-200/70', 'bg-red-50/80', 'px-5', 'py-3', 'shadow-sm', 'dark:border-red-700/60', 'dark:bg-red-900/25')}>
              <div className={cn('text-sm', 'font-medium', 'text-red-600', 'dark:text-red-300')}>Error: {error}</div>
            </div>
          )}
