...
describe('Retry Logic and Exponential Backoff', () => {
  ...
  beforeEach(() => {
    ...
    jest
      ...
      .mockResolvedValue({ message: 'Logged out', cleared_session: 'test' });
    ApiClient.setTestMaxRetries(3);
    setTimeoutSpy = jest.spyOn(globalThis, 'setTimeout').mockImplementation(((cb: TimerHandler) => {
  ...
  describe('HTTP Method Retry Consistency', () => {
    ...
    it('should retry DELETE requests on transient errors', async () => {
      mockHttp.delete.mockRejectedValueOnce(new Error('Request aborted')).mockResolvedValueOnce({});
