  ...
  beforeEach(() => {
    ...
    fetchMock = installFetchRoutes({
      'POST /api/auth/login': (req: Request) => {
        return new Response(JSON.stringify({ token: 'jwt-token' }), { status: 200, headers: { 'Content-Type': 'application/json' } })
      },
    ...
    describe('Given a login screen component', () => {
      describe('When it is rendered', () => {
        it('Then it should show email and password fields with register link', () => {
          ...
          expect(screen.getByRole('button', { name: /register|create account|sign up/i })).toBeInTheDocument()
        })
      })
      ...
      describe('When valid credentials are submitted', () => {
        ...
        fetchMock = installFetchRoutes({
          ...
          await waitFor(() => {
            const call = (fetchMock as any).mock.calls.find((c: any[]) => String(c[0]) === '/api/auth/login')
    ...
    describe('Given a register screen component', () => {
      ...
      describe('When valid registration data is submitted', () => {
        it('Then it should call register API and navigate to login', async () => {
          ...
          await waitFor(() => {
            const call = (fetchMock as any).mock.calls.find((c: any[]) => String(c[0]) === '/api/auth/register')
            expect(call).toBeTruthy()
            const [, init] = call
            expect(init).toMatchObject({
              method: 'POST',
